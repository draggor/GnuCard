...
function makeRadioListCard(json) {
	var radio = $('<input type="radio" name="cardRadio">').val(json.id);
	return makeDraggable(makeHover($('<li>').addClass('ui-state-default').attr({id: json.id, img: json.img}).html($(radio)).append(json.name)));
}
